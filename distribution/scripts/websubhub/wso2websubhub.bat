...
@echo off
rem WSO2 WebSubHub startup script
rem This script starts, stops, and restarts the WSO2 WebSubHub using Java
rem Usage: wso2websubhub.bat {start|stop|restart|status}
...
set "PID_FILE=%BASE_DIR%\wso2websubhub.pid"
...
if exist "%PID_FILE%" (
    for /f %%p in ('type "%PID_FILE%"') do (
        ...
        if not errorlevel 1 (
            echo WSO2 WebSubHub is already running as process %%p
            exit /b 0
        )
    )
)
...
if defined SERVER_PID (
    ...
    echo WSO2 WebSubHub started successfully with process ID %SERVER_PID%
) else (
...
echo Stopping WSO2 WebSubHub ^(PID: %SERVER_PID%^)...
...
if exist "%PID_FILE%" (
    for /f %%p in ('type "%PID_FILE%"') do (
        ...
        tasklist /FI "PID eq %%p" 2>nul | find /I "java.exe" >nul
        if not errorlevel 1 (
            echo WSO2 WebSubHub is running ^(PID: %%p^)
        ) else (
            echo WSO2 WebSubHub is not running
            del "%PID_FILE%" 2>nul
        )
