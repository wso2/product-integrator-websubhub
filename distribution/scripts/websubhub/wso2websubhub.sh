...
# WSO2 WebSubHub startup script
# This script starts, stops, and restarts the WSO2 WebSubHub
# Usage: wso2websubhub.sh {start|stop|restart|status}
...
PID_FILE="$BASE_DIR/wso2websubhub.pid"
...
startServer() {
    if [ -e "$PID_FILE" ]; then
        if ps -p $(cat "$PID_FILE") > /dev/null; then
            echo "WSO2 WebSubHub is already running as process $(cat $PID_FILE)"
            exit 0
        fi
    fi
    ...
    if ps -p $PID > /dev/null; then
        echo "WSO2 WebSubHub started successfully with process ID $PID"
    else
        echo "Failed to start WSO2 WebSubHub"
        rm -f "$PID_FILE"
        exit 1
    fi
}
...
stopServer() {
    if [ ! -e "$PID_FILE" ]; then
        echo "WSO2 WebSubHub is not running"
        return
    fi
    ...
    echo "Stopping WSO2 WebSubHub (PID: $PID)..."
    kill -TERM $PID
    
    # Wait for graceful shutdown
    for i in {1..30}; do
        if ! ps -p $PID > /dev/null; then
            echo "WSO2 WebSubHub stopped successfully"
            rm -f "$PID_FILE"
            return
        fi
        sleep 1
    ...
    echo "Forcing WSO2 WebSubHub shutdown..."
    kill -KILL $PID > /dev/null 2>&1
    rm -f "$PID_FILE"
    echo "WSO2 WebSubHub stopped"
}

# Function to restart the server
restartServer() {
    echo "Restarting WSO2 WebSubHub..."
    stopServer
    sleep 3
    startServer
}
...
if [ "$1" = "start" ]; then
    ...
    if [ -e "$PID_FILE" ]; then
        PID=$(cat "$PID_FILE")
        if ps -p $PID > /dev/null; then
            echo "WSO2 WebSubHub is running (PID: $PID)"
        else
            echo "WSO2 WebSubHub is not running"
            rm -f "$PID_FILE"
        fi
