    ...
    if authHeader is string {
        ...
        if auth is jwt:Payload {
            ...
            if forbiddenError is http:Forbidden {
                common:logRecoverableError("Forbidden Error received - Authentication credentials invalid");
                return error("Not authorized");
            }
        } else {
            common:logRecoverableError("Unauthorized Error received - Authentication credentials invalid");
            return error("Not authorized");
        }
    } else {
        common:logRecoverableError("Authorization header not found");
        return error("Not authorized");
    }
}
