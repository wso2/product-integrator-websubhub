...
// Unit tests for `constructContentDistMsg` — converts a stored storeapi:Message back into a
// websubhub:ContentDistributionMessage, choosing the delivery Content-Type from the
// `x-hub-contentType` metadata key written at ingest.
//
// Ballerina type-system note (why these tests matter):
//   - string <: json and byte[] <: json[]. For any non-JSON type the delivered `content` MUST be
//     the raw byte[] payload — a decoded string would be re-JSON-serialized by the HubClient
//     (adding quotes). The binary and text tests below lock that in.
...
function testConstructContentDist_Json_WithMetadataKey() returns error? {
    ...
    websubhub:ContentDistributionMessage|error result = constructContentDistMsg(makeStoredMessage(payloadBytes, metadata));
...
function testConstructContentDist_Json_NoMetadata_BackwardCompat() returns error? {
    ...
    websubhub:ContentDistributionMessage|error result = constructContentDistMsg(makeStoredMessage(payloadBytes, ()));
